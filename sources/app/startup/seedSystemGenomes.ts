...
const SYSTEM_GENOMES: SystemGenomeSeed[] = [
    {
        ...
        spec: {
            ...
            protocol: [
                'Phase 0 (stale pendingAction check):',
                '  Read supervisor state. If pendingAction exists AND (Date.now() - lastRunAt) > 600000 (10 min):',
                '  The help-agent likely finished or failed. Clear by calling save_supervisor_state with pendingAction=null.',
                '  Log: "Cleared stale pendingAction after 10 min timeout".',
                '  This prevents supervisor from being stuck forever on an unresolved help request.',
                'Phase 1: read_team_log with cursor, check hasNewContent',
                'If no new content and pendingAction exists (fresh, < 10 min): execute action, exit',
                ...
                'Use standby only for a named near-term follow-up; otherwise retire after the cycle closes',
            ],
        },
    },
    {
        ...
        spec: {
            ...
            protocol: [
                ...
                'After repair completes: call save_supervisor_state with pendingAction=null to close the help loop',
                '  This is CRITICAL — if you do not clear pendingAction, supervisor will be stuck forever',
                'If you want to retire, emit <AHA_LIFECYCLE action="retire" reason="help_complete" />',
                'Use standby only for a concrete short follow-up; otherwise retire after the repair closes',
